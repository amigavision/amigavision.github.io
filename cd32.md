---
title: AmigaVision CD³² Setup for MiSTer
description: A ready-to-go package for playing Amiga CD32 games on MiSTer’s Amiga core.
--- 

If you are unfamiliar with the Amiga CD32, it was essentially an Amiga 1200 with a 2× speed CD drive, packaged with gamepads and in a console form factor. It was released in September 1993. 

![Amiga CD32](https://amiga.vision/images/cd32.png)

While the CD32 never really got its time to shine because of Commodore's bankruptcy soon after launch, there are some fun expansions of existing Amiga games with great CD audio and Full Motion Video intros, so some of its ~150 games are worth checking out.
...
* Games that can make use of AmigaVision's [5×PAL Overscale presets] — like Chaos Engine CD32 — now use that on a per-game basis when a game can handle it.
...
If you are not using an external USB drive or Network Attached Storage via SMB (NAS) to store your game setup, you can delete `USB.zip` and `NAS.zip`.

If you *are* using an external USB drive or NAS for storing your game setup, there are included `USB.zip` and `NAS.zip` files included that you should use for those setups instead — these unpack in the same way as the main archive, and you copy these files to your MiSTer USB or NAS drive.
...
If you are one of the sickos 😅 that want to run SNES at 50hz PAL in 1200p, there's an easy solution — do a per-system setting for *just those* systems to override the global default. For the CD32 core to work with the MGL approach without adding 100s of configuration settings to `MiSTer.ini`, it will have to be the global default PAL setting.
...
## 🗺️ Mapping Controller Inputs {#controllers}
...
## 🕹️ Game Recommendations {#recommendations}

<iframe style="aspect-ratio: 16/9; width: 100%;"
        src="https://www.youtube.com/embed/T8iMcCmK6MI"
...
* **Castlevania** — Incredible modern CD32 implementation of a Castlevania game. Not a port, but its own independent game, worth playing for the music alone!
...
* **Disposable Hero** — One of Amiga's best shoot'em-ups, and the CD soundtrack elevates this version even further.
...
* We generate 1935 config files to ensure these CD32 games work on MiSTer.
* We generate 10 different hard disk images to power these.

## 🐭 Games That Still Require a Mouse {#mouse}
...
## ↕️ 68 Games Will Use 5×PAL Overscale {#overscale}
...
Compatibility isn’t perfect — some games may not work or may lack their CD audio tracks.
...
## 💿 Unique CD³² Improvements {#unique}

[Sourced from the GameFAQs forums](https://gamefaqs.gamespot.com/boards/951940-amiga-cd32/80110566), with some entries added.

69 games take advantage of the CD32 in some way over their Amiga 500/1200 counterparts, the rest are identical to their floppy disk versions.
...
### Extra levels (5 games)

* Brian the Lion 
* Clockwiser: Time is Running Out...
* Emerald Mines
* Zool — Extra world at the end of the game
* Zool 2 — Extra world at the start of the game before the usual first world (Swan Lake)

### Remastered (5 games)

* Fantastic Dizzy — remastered in 256 colors
* Wing Commander — remastered in 256 colors
* Vital Light — remastered in 256 colors
* Kid Chaos — remastered in 256 colors
* Fire & Ice — additional parallax backgrounds
...
## 📝 Addendum: File Naming Convention {#naming}
...
[https://amiga.vision/overscale]:https://amiga.vision/overscale
[https://amiga.vision/cd32]:https://amiga.vision/cd32
[5×PAL Overscale presets]:https://amiga.vision/overscale
