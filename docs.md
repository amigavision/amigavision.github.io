...
* The core does not yet support the new file naming scheme, so rename `AmigaVision.hdf` to `MegaAGS.hdf` — and, if you don't already have an existing save file, rename `AmigaVision-Saves.hdf` to `MegaAGS-Saves.hdf`.
