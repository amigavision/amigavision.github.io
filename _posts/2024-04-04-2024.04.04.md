...
* To solve issues with [Super Hi-Res sprites](https://github.com/amigavision/AmigaVision/issues/158), 28MHz is now the new default pixel clock — which means **it is extra important that you update the `Amiga.cfg` file and `MiSTer.ini` settings if you are upgrading**. You should always replace all files (except your `Saves.hdf`) when upgrading, and this release is no exception. Back up your existing file if you have custom settings, and re-apply them once everything is working.
